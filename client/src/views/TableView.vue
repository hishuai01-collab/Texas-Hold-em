...
<template>
  <main class="ui-page px-3 py-4 sm:px-6 sm:py-6">
    <div class="mx-auto max-w-6xl">
      <!-- ── 头部 ── -->
      <header class="mb-4 flex flex-col gap-3 border-b border-gray-700/30 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <RouterLink class="font-mono text-[10px] tracking-[.24em] text-gray-500 hover:text-gray-300" :to="{ name: 'room-list' }">← 房间选择</RouterLink>
        ...
        <div class="flex flex-wrap items-center gap-2 text-xs">
          ...
          <span class="rounded-full bg-gray-900 px-3 py-1 text-gray-400">事件 #{{ pokerStore.lastSeq }}</span>
           <span v-if="pokerStore.playerId" class="rounded-full bg-gray-900 px-3 py-1 text-gray-300">已入座</span>
          ...
          <section v-if="!showConnectPanel" class="ui-panel space-y-2 p-4">
            <div class="flex items-center justify-between text-xs">
              <span class="font-mono text-gray-400">玩家</span>
               <span class="font-mono text-gray-300">{{ pokerStore.playerName }}</span>
            </div>
            <button
