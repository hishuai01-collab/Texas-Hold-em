...
import { IpBlocklist, extractClientIp } from './infrastructure/security/IpBlocklist.js';
import { guardAction, type ConnectionState } from './infrastructure/security/ConnectionState.js';
...
import { PokerMetrics, startMetricsServer } from './infrastructure/observability/PrometheusMetrics.js';
import { TelegramNotifier } from './infrastructure/observability/TelegramNotifier.js';
import { TelegramAdminBot, type AdminBotDependencies } from './infrastructure/observability/TelegramAdminBot.js';
...
const blocklist = new IpBlocklist({ strikesToBan: STRIKES_TO_BAN, banTtlMs: BAN_TTL_MS, capacity: 10_000 });
...
/** 向前端发送统一结构化错误（code 枚举 + 用户语言文案，绝不暴露后端字段/栈） */
function sendError(ext: ExtWebSocket, code: string | undefined, message: string): void {
  if (ext.readyState === WebSocket.OPEN) {
    ext.send(JSON.stringify({ type: 'ERROR', code, message } satisfies ServerMsg));
  }
}
...
/** 多桌路由：tableId 取自连接握手 URL 的 query（?tableId=xxx），非法或缺省一律回落默认桌，兼容未改造的旧前端。 */
function parseTableId(url: string | undefined): string {
  if (!url) return DEFAULT_TABLE_ID;
  try {
    const parsed = new URL(url, 'http://internal');
    const raw = parsed.searchParams.get('tableId');
    if (raw && TABLE_ID_PATTERN.test(raw)) return raw;
  } catch {
    /* 畸形 URL：回落默认桌 */
  }
...
const tableManager = new TableManager({ eventStore, registry: tableRegistry, metrics });
...
function cleanupPlayer(authId: string | null, ws: ExtWebSocket, session: TableSession): void {
...
async function main(): Promise<void> {
  ...
  const knownTableIds = await tableRegistry.listActive().catch(error => {
    console.error('[bootstrap] table registry listActive failed (continuing with only the default table)', error);
    return [] as string[];
  });
  for (const tableId of knownTableIds) {
    ...
    try {
      ...
      console.error(`[bootstrap] warm-up failed for table ${tableId} (will hydrate lazily on first connection)`, error);
    }
  }
  ...
  const wss = new WebSocketServer({
    ...
    verifyClient: (info: { req: IncomingMessage }, cb) => {
      const ip = extractClientIp(info.req.headers['x-forwarded-for'] as string | undefined, info.req.socket.remoteAddress);
      if (blocklist.isBlocked(ip)) {
        cb(false, 403, 'Forbidden: your IP is temporarily blocked due to suspicious activity');
        console.warn(JSON.stringify({ type: 'WSS_HANDSHAKE_BLOCKED', ip, at: new Date().toISOString() }));
        return;
      }
      cb(true);
    },
  ...
  wss.on('connection', (ws, req) => {
    ...
    if (!acceptingConnections) { ext.close(1012, 'server shutting down'); return; }
    ext.isAlive = true;
    ext.authId = null;
    ext.connState = 'CONNECTING';
    ext.tableId = parseTableId(req.url);
    ext.clientIp = extractClientIp(req.headers['x-forwarded-for'] as string | undefined, req.socket.remoteAddress);
    ext.gateway = new WebSocketGateway((event) => {
      ...
      console.warn(JSON.stringify({
        ...event, ip: ext.clientIp, tableId: ext.tableId, banned: bannedUntil !== null, at: new Date().toISOString(),
      }));
    });
    ...
    const handleMessage = (raw: unknown): void => {
      ...
      if (!inspected.ok) {
        if (inspected.disconnect) ext.close(1008, 'protocol violations exceeded');
        else sendError(ext, 'PROTOCOL_VIOLATION', `协议安全校验失败: ${inspected.reason}`);
        return;
      }
      ...
      if (!parsed.success) {
        const bannedUntil = blocklist.strike(ext.clientIp);
        console.warn(JSON.stringify({
          type: 'WSS_SECURITY_REJECTED', reason: 'SCHEMA_INVALID', violations: -1,
          disconnect: false, ip: ext.clientIp, banned: bannedUntil !== null,
          issue: parsed.error.issues[0]?.message, at: new Date().toISOString(),
        }));
      ...
      switch (msg.type) {
        case 'JOIN': {
          if (ext.authId) { sendError(ext, undefined, '连接已认证，不能重复入座'); break; }
          const id = randomUUID();
          s.runCommand(() => {
            try {
            ...
            refreshConnectionGauge();
            tableManager.refreshOnlineTablesGauge();
            s.broadcastSessionEvent({ type: 'PLAYER_JOINED', seq: 0, seats: s.engine.views() });
            void reconnectTokens.issue({ playerId: id, tableId: ext.tableId }).then(token => {
              if (ext.readyState === WebSocket.OPEN && ext.authId === id) {
                ext.send(JSON.stringify({ type: 'JOINED', seq: 0, you: id, reconnectToken: token, seats: s.engine.views() } satisfies ServerMsg));
              }
            }).catch(error => {
              console.error('[reconnect] token issue failed', error);
              ext.send(JSON.stringify({ type: 'ERROR', message: '重连令牌签发失败' } satisfies ServerMsg));
            });
          });
          break;
        }
        case 'LEAVE': {
          const id = ext.authId;
          if (!id) { sendError(ext, undefined, '尚未入座'); break; }
          s.runCommand(() => {
            try {
            ...
            refreshConnectionGauge();
            tableManager.refreshOnlineTablesGauge();
            s.broadcastSessionEvent({ type: 'PLAYER_LEFT', seq: 0, playerId: id, reason: 'LEFT', seats: s.engine.views() });
          });
          break;
        }
        case 'START':
        ...
        case 'ACTION': {
          const id = ext.authId;
          if (!id) { sendError(ext, 'NOT_AUTHORIZED', '连接未认证'); break; }
          const seat = s.engine.seats.find(p => p.id === id);
          const isActive = s.engine.activePlayer === id;
          const toCall = seat ? s.engine.currentBetAmount - seat.betThisStreet : 0;
          ...
          if (pre.error) { sendError(ext, pre.error.code, pre.error.message); break; }

          if (!verifyActionSignature(
            { playerId: id, clientSeq: msg.clientSeq, action: msg.action, amount: msg.amount ?? 0, tableEpoch: s.tableEpoch },
            s.tableEpoch,
            (msg as ClientMsg & { sig?: string }).sig,
          )) {
            console.warn(JSON.stringify({ type: 'ACTION_SIG_REJECTED', tableId: s.tableId, playerId: id, at: new Date().toISOString() }));
        ...
        case 'RECONNECT': {
          ...
          void reconnectTokens.consume(msg.reconnectToken).then(record => s.runCommand(() => {
            if (!record || record.playerId !== msg.playerId || record.tableId !== s.tableId || !s.engine.seats.some(p => p.id === msg.playerId)) {
              ext.send(JSON.stringify({ type: 'ERROR', message: '重连校验失败' } satisfies ServerMsg));
              return;
            }
            const previous = s.sockets.get(msg.playerId);
            if (previous && previous !== ext) previous.close(1012, 'replaced by reconnect');
            ext.authId = msg.playerId;
            ext.connState = 'CONNECTED';
            s.sockets.set(msg.playerId, ext);
            s.timeoutManager.markConnected(msg.playerId);
            refreshConnectionGauge();
            const events = s.engine.getReplay(msg.playerId, msg.lastSeq);
            ext.send(JSON.stringify({ type: 'EVENT_REPLAY', events } satisfies ServerMsg));
            void reconnectTokens.issue({ playerId: msg.playerId, tableId: s.tableId }).then(token => {
              if (ext.readyState === WebSocket.OPEN && ext.authId === msg.playerId) {
                ext.send(JSON.stringify({ type: 'RECONNECTED', seq: 0, reconnectToken: token, seats: s.engine.views() } satisfies ServerMsg));
              }
            });
          })).catch(error => {
            console.error('[reconnect] token consume failed', error);
            ext.send(JSON.stringify({ type: 'ERROR', message: '重连服务不可用' } satisfies ServerMsg));
          });
          break;
        }
      }
    ...
    tableManager.getOrCreate(ext.tableId).then(s => {
      session = s;
      ready = true;
    ...
    ext.on('message', raw => {
      if (!ready) { pendingRaw.push(raw); return; }
      handleMessage(raw);
    });
    ext.on('close', code => {
  ...
  const httpApi = new HttpApiServer({ tableManager });
  ...
  console.log(`poker server on http://${HOST}:${PORT}  HTTP API on http://${HOST}:${HTTP_API_PORT}  metrics on ${process.env.METRICS_PORT ?? 9091}  (BOTS=${BOTS}, tables=${tableManager.size})`);
  ...
  if (adminTgToken && adminTgId) {
    ...
    void TelegramAdminBot.startAdminBot({ botToken: adminTgToken, adminTgId, deps: adminBotDeps });
  }
}
